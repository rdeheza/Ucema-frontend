...
import getProgrammedClasses from '../../shared/services/Career/Programmed';
    ...
    <>
        <View style={[styles.item]}>
            <View style={[styles.courseTitleWrapper]} >
                ...
                <Text style={[styles.courseTeacher, styles.textWhite]}>{item.desc_espacio} (Sede: {item.sede})</Text>
                ...
                token.then( tokenFounded => {
                    if ( tokenFounded ) {
                        userid.then( userIdFounded => {
                            if ( userIdFounded ) {
                                getProgrammedClasses(userIdFounded, careerData[0].identificacion, careerData[0].programa, careerData[0].orientiacion, Moment().year(), careerData[0].descred, tokenFounded, userFounded)
                                    .then( response => {
                                        if ( response.status != 200 ) {
                                            setForceCloseApp();
                                        } else if ( response.data.data ) {
                                            setProgrammedClasses(response.data.data);
                                            setLoading(false);
                                        } else {
    ...
    return (
        <SafeAreaView style={ styles.container }>
            ...
            <View style={[ styles.container, styles.justifyContainer]}>
                <Text style={[ styles.textCapitalize, styles.textItalic, styles.textBold, styles.title, { marginBottom: 20}]}>clases programadas</Text>
                ...
                { isLoading 
                    ? 
                        ...
                        /> 
                    :
                        (programmedClasses.length > 0)
                        ?
                        <FlatList
                            data={ programmedClasses }
                            renderItem={ renderItem }
                            keyExtractor={ item => `${item.desc_curso}${item.f_cla}${item.desc_curso}` }
                        />
                        :
                        <Text style={{ flex: 1 }}>No hay clases programadas hasta el momento.</Text>
                }
